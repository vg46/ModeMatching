% BeamtoCryo with additional lens(es) between Z4 and ITM
    % Z distances of Z3, Z4 and ITM slightly modified


goo = beamPath;

% Inputs of cavity parameters (length d = 9.97cm, mirror curvatures R1 = 10cm and R2 =
    % flat (infinite)
d = 0.0997;
R1 = -0.1;                 %IS THE SIGN CORRECT
R2 = Inf;
...
%lens in here
zM3=12.2; % Third steering mirror
zM4=13.7; % Fourth steering mirror
%lens(es) in here
zCav=14.9-ITM_thickness; %Distance to first cavity mirror (curved)
zCavEnd= 14.9+d; %Distance to end of cavity mirror (flat)
...
%radiusofcurvatureListend = [75, 100, 200, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN] *(1e-3);   % End lenses unlikely to have a focal length longer than 250mm.
%focalLengthListend = [86.4023, 114.8271, 228.5356, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN] *(1e-3); 
%focalLengthListend = [86.4023, 114.8271, 228.5356] *(1e-3);
%lensListend = component.lens(focalLengthListend);

% 5 lenses, one between collimator output and 45 degree mirror; one between
    % ZM2 and ZM3; three between ZM4 and cavity. Positions will be optimised
...
goo.addComponent(component.lens(0.200,zCav-0.2,'L4'));
%goo.addComponent(component.lens(0.200,zCav-0.2,'L5'));
...
[pathList,overlapList] = goo.chooseComponents(...
                ...
                'L4',lensList.duplicate,[zM4+0.02 zCav-0.02],...
                '-vt',.001);
           % 'L5',lensListend.duplicate,[zM4+0.02 zCav-0.02],...
